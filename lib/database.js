...
var Database = module.exports = function (type) {
    ...
    switch (type) {
        ...
        case 'test': break;
        default:
            throw new Error('Error: Database type "' + type + '" not supported');
            process.exit(1);
            break;
    }
    ...
    try {
       ...
       throw new Error('Error: Optional dependency "' + type + '" not installed.\nIt can be installed by running: npm install ' + type);
       process.exit(1);
    }
...
Database.prototype.exec = function (action, args, cb) {
    ...
    _this.checkAccessor(action, function (err) {
        ...
        // define the generic callback wrapper for all action responses
